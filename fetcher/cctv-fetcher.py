...
FETCH_INTERVAL = int(os.environ.get("FETCH_INTERVAL", "86400"))  # seconds
...
def main():
    while True:
        try:
            ...
            logger.info("Wrote %d cameras to %s", len(cameras), CAM_DATA_PATH)
        except Exception:
            logger.exception("Fetch failed; keeping previous file")
